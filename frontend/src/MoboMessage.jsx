...
export default function MoboChat() {
    ...
    function sendToFriend() {
        if (message.length !== 0 && availableUsers.length !== 0 && availableUsers.some((val) => { return val.userId === id })) {
    ...
    useEffect(() => {
        if (myId !== undefined) { socket.current.emit('addUser', myId); }
    }, [myId])
    useEffect(() => {
        if (id.length !== 0) {
            sendId();
                    ...
                    {(DbMessage !== null) ? <>
                        <div className="user">
                            ...
                            <div className="action">
                                <i className="bi bi-chat-right-quote" style={{ color: "blueviolet" }}></i>
                        ...
                        <div className="messages" id='message'>
                            {(DbMessage.length !== 0) ? DbMessage.map((val, lengthMap) => {
                                return (<React.Fragment >
                                    <div className={(val.type === 'sended') ? 'right' : 'left'}>
                                        {(val.type !== 'sended') ? <Avatar name={id} /> : ((lengthMap === lengthOfMessage && val.status === 'seen') ? <div className='seen'><Avatar name={id}></Avatar></div> : null)}
                                        <p className={val.type}>{val.message}</p>
                                        <div className="deleteAction">
                                            <input type="button" value="Delete" onClick={() => { deleteMessage(lengthMap); console.log('deleted') }} />
                                        </div>
                                    </div>
                                </React.Fragment>);
                            }) :
                                <div className="noMessages">
                                    <p className='selectOthers' style={{ fontFamily: 'Oswald' }}>No Conversation Yet with {name}, Start Convo!</p>
                                </div>
                            }

                        </div>
                        <div className="sendMessages">
                            <input type="text" placeholder="Write a Message" id="data" autoComplete='off' onChange={(e) => { setMessage(e.target.value) }} autoFocus={true} onBlur={({ target }) => target.focus()} />
                            <img src={send} alt="sendMessage" style={{ height: "20px", width: "20px", marginLeft: '-5%' }}
                        ...
                        <div className="selectOne">
                            <p className='notSelected' style={{ fontFamily: 'Oswald' }}>Please Select Anyone From Left To Chat!</p>
                        </div>
                    }
                </div>
            </section>
