...
export default function Chat() {
    ...
    return (<>
        <div className="logoutAll" style={{ display: 'flex', justifyContent: 'center', position: 'absolute', zIndex: 10,marginTop:'1%'}}>
            <h1 className="logout" style={{ marginLeft: '72vw' }}>Logout</h1><i class="bi bi-box-arrow-right" id="logoutIcon"
                style={{ color: 'blue', marginLeft: '1%' }} onClick={()=>{history.push('/logout')}}></i>
        </div>
        <div className='all'>
            <section>
                <div className="dashboard" id="dashboard">
                    <div className="head">
                        ...
                        <i className="bi bi-chat-right-text-fill" style={{ color: "blueviolet" }}></i>
                    </div>
                    <Users id={idSet} state={state} online={availableUsers}/>
                </div>
                <ChatMessage name={name} id={id} myId={myId} messageSet={gettingMessage} newMessage={newMessage} />
            </section>
        </div>
    </>);
}