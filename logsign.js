...
const signupSchema = new db.db.Schema({
    name: {
        type: String,
        required: true
    },
...
const chatSchema = new db.db.Schema({
    _id: {
        type: String,
        required: true
    },
...
const Data = new db.db.model('user', signupSchema);
    ...
    if (!name || !email || !password) {
        ...
        try {
            ...
            if (!userSignup) {
                ...
                res.status(403).json({ message: 'Provided Email Is Already Registered' })
            }
        }
        catch (err) {
        ...
        try {
            ...
            if (!userLogin) {
                ...
                if (!isMatch) {
                    ...
                    const Chat = new db.db.model(`${userLogin._id}`, chatSchema);
                    const chatMessage = new Chat({ _id: userId, messages: [{ type: 'sended', message: "hello" }] });
                    await chatMessage.save().then(() => { console.log('done') }).catch((err) => { console.log('This is the error here ' + err) });
...
app.post('/chat', Authenticate, async (req, res) => {
    try {
        ...
        console.log(req.body)
        const MyChat = new db.db.model(`${myId}`, chatSchema);

        // For only Creating My collection In User Collec
        const UserChat = new db.db.model(`${id}`, chatSchema);
        ...
        if (!chatData) {
            ...
            UserChat.updateOne({ '_id': myId }, { $set: { [`messages.${length}.status`]: 'seen' } }, (err, res) => {
...
app.post('/chatMessage', Authenticate, async (req, res) => {
    try {
        ...
        const MyChat = new db.db.model(`${myId}`, chatSchema);
        const FriendChat = new db.db.model(`${id}`, chatSchema);
        ...
        MyChat.findByIdAndUpdate({ _id: id }, { $push: { 'messages': { type: "sended", message, status: "unseen", messageId: (lengthOfMessage + 1) } } }, { new: true, useFindAndModify: false })
            .then((respond) => res.status(200).send(respond)).catch(err => console.log('error occured on myChat update ' + err));
        //console.log('res from mongodb update ' + res.messages);

        FriendChat.findByIdAndUpdate({ _id: myId }, { $push: { 'messages': { type: "receive", message, status: "unseen", messageId: (lengthOfMessage + 1) } } }, { new: true, useFindAndModify: false }, (err, res) => {
...
app.post('/deleteMessage', Authenticate, async (req, res) => {
    try {
        ...
        MyChat.findOneAndUpdate({ $and: [{ _id: id }, { 'messages.messageId': messId }] }, { $set: { 'messages.$.message': 'User Unsent Message' } }, { new: true, useFindAndModify: false })
            .then((respond) => res.status(200).send(respond)).catch(err => console.log('error occured on deleting my own Message ' + err));

        FriendChat.updateOne({ $and: [{ _id: myId }, { 'messages.messageId': messId }] }, { $set: { 'messages.$.message': 'User Unsent Message' } }, (err, res) => {
